...
| Local schema markup | 10% | good | 88 | Connected `@graph`; correct subtype, address, phone, hours, `areaServed`, credential, `sameAs`, `geo`, `priceRange`, `image`/`logo`, `@id`, E.164 phone, and region-scoped `Service` nodes on all three published hubs. Review schema remains deliberately omitted pending live GBP verification. |
...
| Per-hub area targeting | ✅ **Shipped 2026-09-07 via #31 (D-136).** Each published hub now asserts its region through a connected `Service` node. | `buildHubServiceNode()` in `lib/schema.ts` emits `AdministrativeArea` `areaServed` with the region name and `provider` → `#business`. Served HTML verified for Hills District, Eastern Suburbs and North Shore. |
...
| **4** | ✅ **DONE 2026-09-03 (issue #35, D-121)** | **De-template the 3 published hubs** so each passes the swap test: a unique 2–3 paragraph section on renovating *in that area* (heritage/Federation plumbing, strata/apartment access, slab vs timber substrate, terrace parking/skip access — all truthful, none suburb-specific-invented), a 4–6 question **local FAQ** with `FAQPage` schema, a link to `/packages/`, and a testimonial. **Shipped:** new `lib/hubContent.ts` (per-region answer-first lead 134–167 words, "Renovating a bathroom in {region}" local-detail section, 5-question local FAQ, pinned verbatim testimonial); rendered by `app/services/[slug]/[location]/page.tsx`; `service-areas.json` `updated` bumped. Swap test passes; readability 70–73; FAQ schema byte-matches visible `<details>`; browser-verified desktop + 390px. | #2 (clean links first) | L | `app/services/[slug]/[location]/page.tsx`, `lib/hubContent.ts`, `lib/reviews.ts`, `service-areas.json` |
...
- [x] Action #4 (de-template the 3 hubs) implemented — issue #35, 2026-09-03, D-121
