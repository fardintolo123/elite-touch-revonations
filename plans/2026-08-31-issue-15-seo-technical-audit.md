...
- [x] **#18 · L-1 + L-4** — **L-4 shipped** in commit `b21c322` (`Host:` line removed from `app/robots.ts`; verified absent from `/robots.txt` via `next dev`). **L-1 closed WONTFIX 2026-09-10**: the 404 carries two `<meta name="robots">` tags — our explicit `noindex, follow` plus a `noindex` Next auto-injects for every 404-status response. Both are noindex (zero indexation risk, purely cosmetic). Collapsing to one would require either dropping the sitewide `index, follow` layout default (touches every page, violates `CLAUDE.md` Architecture Rules) or a deeper Next metadata change — both exceed the scope and risk/benefit for a cosmetic issue. Issue closed per investigation findings (2026-09-07, 2026-09-10). *(Phase 1, steps 2–3.)*
