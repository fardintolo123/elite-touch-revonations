...
|---|---|---|---|---|---|---|---|---|---|
| 2026-09-20 | Published location heroes (issue #63 / D-146) — fresh production build, served HTML and Playwright desktop + 390px checks | n/a | n/a | n/a | n/a | n/a | n/a | **no JS/dependency/client-boundary change; no new image asset** | The seven published location routes already rendered their first local project photo as a high-priority lower-page image. LocationHero moves that same image into the full-bleed hero and the lower proof image is now lazy, so each route still has exactly one high-priority image and the request count/asset stays the same. next/image remains responsible for responsive WebP srcsets. No Lighthouse number was added because the render-path image request is a priority-placement handoff rather than an added resource; representative browser checks confirmed the image loaded and no horizontal overflow. |
| 2026-09-14 | **Live production domain**, real Google PageSpeed Insights API run (owner-supplied API key, one-off local `.env.local` use — never committed), issues #26/#27 | **home mobile 96 · home desktop 100 · gallery-detail mobile 99** | home mobile 0.9s · desktop 0.3s · gallery 0.9s | home mobile **2.1s** · desktop 0.4s · gallery mobile **2.1s** | home mobile **190ms** · desktop 50ms · gallery mobile 70ms | 0 (desktop 0.015) | home mobile 1.0s · desktop 0.5s · gallery mobile 1.0s | home mobile total page weight **610 KiB** (script 475 KB, of which **third-party 315 KB**: `gtm.js` 125 KB + `gtag/js` 190 KB) | **First real Lighthouse run against the live domain — the 2026-08-23 and 2026-09-07 rows' "still owed" re-measure is now done.** Every §1 CWV target is met with margin: LCP ≤2.5s ✅, CLS ≤0.05 ✅ (0 on mobile), TBT ≤200ms ✅ (190ms, right under the target line, well under the 300ms hard cap), Speed Index ≤4.0s ✅. Total page weight (610 KiB) is well under the ≤1.2 MB mobile-home budget. **One budget line is confirmed over cap, as the 2026-09-01 row predicted it would be once GTM activated:** third-party JS is 315 KB vs the §1 ≤250 KB line — GTM (`gtm.js`, 125 KB) + the GA4 tag it loads (`gtag/js`, 190 KB). This is the accepted D-112 architecture (one measurement path, GTM-only, `afterInteractive`) doing its job; the CWV outcome it exists to protect (TBT, INP-proxy) is still within target, so this is recorded as a **known, accepted trade-off**, not a new open item — slimming it further is only worth doing if a future run shows TBT actually regressing past 200ms. **CrUX field data: none on any URL** (`loadingExperience` absent from every response) — Google's Chrome UX Report requires enough real Chrome users in a rolling 28-day window before it publishes anything for an origin; a brand-new low-traffic domain like this one simply doesn't have that yet. This is a traffic-volume fact, not a credentials or measurement failure — re-check in a few months once real visits accumulate, don't re-attempt sooner expecting a different result. Full JSON responses saved to session scratchpad, not the repo (no reason to commit raw API dumps). |
| 2026-09-07 | `img sizes` correction on the homepage hero, `/gallery/` cards, and `/gallery/{slug}/` photo grid (issue #27 comment) — no fresh Lighthouse run | n/a | n/a | n/a | n/a | n/a | n/a | **no JS change; image-request weight only, and only downward** | Real rendered widths measured live with Playwright against the local dev server at 390/768/1280/1920px (dev server only — no production re-baseline, so no Perf/FCP/LCP/TBT/CLS/SI numbers recorded here; see §5, "a dev server cannot verify production behaviour"). All three `sizes` attributes were wrong in the same direction — uncapped `vw` past the point the CSS grid/container actually stops growing — so the browser was requesting a wider `srcset` rung than the layout ever renders, worst case for the gallery-detail LCP image (`100vw` claimed at every breakpoint; real render caps at 1200px, the `--et-container-max` content width). Fixed to capped `px` values matching the measured caps: homepage hero `483px` (≥1024px) / `100vw`; gallery index cards `344px` (≥1024px) / `664px` (≥768px) / `100vw`; gallery-detail wide (LCP) image `1200px` (≥1024px) / `100vw`; gallery-detail grid image `588px` (≥768px) / `100vw`. This can only reduce or hold flat the downloaded LCP-image byte size on desktop viewports — it cannot regress any §1 metric. A full Lighthouse re-run against the live domain is still owed (see the 2026-08-23 and 2026-09-07 live-domain rows below) and will reflect this fix once run. Also checked while in the browser: `.et-body-sm` (14px / 21.7px line-height, ratio 1.55) and `.et-caption` (13px / 18.2px, ratio 1.4, used only for a short "Completed by 2026" label) at 390px — both read comfortably, no change made. |
| 2026-09-07 | Attempted live-domain PSI/CrUX run for issue #27 finding L-8 | — | — | — | — | — | — | — | **Still blocked** — no Google API credentials configured in this session, and the public `pagespeed.web.dev` report page is a JS app that returns only a "loading" shell to a non-interactive fetch. No field or lab numbers obtained. Needs a session with Google API credentials, or a person running PageSpeed Insights manually in a browser against the homepage + 2–3 inner pages and recording the result here. |
| 2026-09-04 | Sitewide `og:image`/`twitter:image` (issue #20 / H-3) — no fresh Lighthouse run | n/a | n/a | n/a | n/a | n/a | n/a | **0 KB added to any page's render path** | `og:image`/`twitter:image` are `<meta>` tags a browser never fetches during a normal page load — only social/messaging crawlers request the referenced file, out-of-band, when generating a link preview. The one new asset (`public/og/default.jpg`, 64 KB JPEG, rule 19's "dedicated 1200×630 crop") is not linked from any page's HTML `<img>`/CSS and is not part of First Load JS, LCP, or any Lighthouse metric. No script, font or client-boundary change. A re-measure was skipped as structurally inapplicable rather than run for form — every §1 metric this table tracks is about what a real visitor's browser downloads, and this change adds nothing to that. |
| 2026-09-01 | `/services/powder-room-renovations/`, isolated local production build of last-green commit `50ca5b7`, mobile, simulated throttling, **GTM env var SET** | 94 | 1.0s | 2.4s | ~190–260ms (clean runs); 145–486ms over 9 runs | 0 | 2.1s | ~465 KB script transfer, of which **~317 KB is the GTM container** | Retroactive check of the GitHub issue #12 powder-room content change (D-107). **With `NEXT_PUBLIC_GTM_ID` set (D-112), third-party JS is ~317 KB — over the §1 ≤250 KB budget**, and TBT approaches the 300 ms hard cap under machine load. GTM is not yet live on Vercel (D-112's env-var step is outstanding) — this row is the *post-activation* picture. Median of 9 runs on a heavily loaded shared machine; 3 runs were VM-noise outliers (perf 69/85/89) discarded per §5. |
| 2026-09-01 | Same page & build as the row above, but **GTM env var UNSET** (= current production reality — Vercel has no `NEXT_PUBLIC_GTM_ID`) | 96–97 (median 96.5) | 0.9–1.0s | 2.2s (clean); 2.5s median | 89–92ms | 0 | 0.9–1.5s | **148 KB script transfer** (10 files); total page 275 KB; 1 preloaded font (26 KB) | The issue #12 change is text + `FAQPage` JSON-LD only — **zero JS/image/font/dependency added**, so First Load JS, LCP-image weight and third-party JS are structurally unchanged by it. Every §1 gate and payload budget passes with margin; the new "about" paragraph, all 4 FAQ questions and the `FAQPage` block confirmed present in served HTML. No regression from D-107. Median of 8 runs; 3 VM-noise outliers (LCP ~3.8s) present in the spread. Comparable to the 2026-08-23 baseline conditions (also GTM-free). |
