...
| K15 | ⚠️ **NEW 2026-08-25 — all 11 `/gallery/{slug}/` pages fail the site's own Flesch ≥60 readability bar** (docs/CONTENT_QUALITY_CHECKLIST.md §2), the 5 original projects included (53.4–55.3) as well as the 6 added this session (49.0–51.8). Discovered while adding the new projects; `scripts/check-readability.mjs`'s route list was extended to cover the 6 new pages, which is how this was caught (D-96/D-97 previously reported "18 of 18 pass," so either the shared template regressed since, or per-project detail pages weren't actually in scope then). All 11 pages share the same "What every job includes" block (waterproofing/warranty/licence copy) — that shared block, not the per-project blurbs, is the likely driver, since it fails identically on old and new pages. **Not fixed here** — it's a sitewide template issue touching legally-significant trust-signal wording, out of scope for a photo-intake task and worth its own readability pass like D-96/D-97 | Agent — needs a dedicated pass |
